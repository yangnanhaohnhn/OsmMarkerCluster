...
package com.osm.mapapi.clusterutil.clustering.view;
...
import com.osm.mapapi.clusterutil.projection.SphericalMercatorProjection;
import com.osm.mapapi.clusterutil.ui.IconGenerator;
import com.osm.mapapi.clusterutil.ui.SquareTextView;
...
import cn.gistone.osmmarkercluster.R;

import static com.osm.mapapi.clusterutil.clustering.algo.NonHierarchicalDistanceBasedAlgorithm.MAX_DISTANCE_AT_ZOOM;


/**
 * The default view for a ClusterManager. Markers are animated in and out of clusters.
 * ClusterManager的默认视图。标记在集群内外都是动态的。
 */
public class DefaultClusterRenderer<T extends ClusterItem> implements ClusterRenderer<T> {
     ...
     */
    private SparseArray<Drawable> mIcons = new SparseArray<Drawable>();
    ...
    @Override
    public void onAdd() {
        mClusterManager.getMarkerCollection().setOnMarkerClickListener((marker, mapView) ->
                mItemClickListener != null && mItemClickListener.onClusterItemClick(mMarkerCache.get(marker)));


        mClusterManager.getClusterMarkerCollection().setOnMarkerClickListener((marker, mapView) ->
                mClickListener != null && mClickListener.onClusterClick(mMarkerToCluster.get(marker)));
    ...
    private class ViewModifier extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            renderTask.setMapZoom(mMap.getZoomLevel());
    ...
    private class RenderTask implements Runnable {
        ...
        public void run() {
            ...
            final BoundingBox visibleBounds = mMap.getBoundingBox();
...
//                if (MapUtils.compareLat(mClusterManager.getContext(), mMap, c.getPosition())) {
                boolean onScreen = visibleBounds.contains(c.getPosition());
                if (zoomingIn && onScreen && SHOULD_ANIMATE) {
                    Point point = mSphericalMercatorProjection.toPoint(c.getPosition());
                    Point closest = findClosestCluster(existingClustersOnScreen, point);
                    if (closest != null) {
                        GeoPoint animateTo = mSphericalMercatorProjection.toLatLng(closest);
...
//            }
    ...
    private class MarkerModifier extends Handler implements MessageQueue.IdleHandler {
         ...
         */
        public void animate(MarkerWithPosition marker, GeoPoint from, GeoPoint to) {
            lock.lock();
         ...
         * @param to     the position to animate to.
         */
        public void animateThenRemove(MarkerWithPosition marker, GeoPoint from, GeoPoint to) {
            lock.lock();
     ...
     */
    protected void onBeforeClusterItemRendered(T item, Marker marker) {
    }
     ...
     */
    protected void onBeforeClusterRendered(Cluster<T> cluster, Marker marker) {

        int bucket = getBucket(cluster);
        Drawable descriptor = mIcons.get(bucket);
        if (descriptor == null) {
            mColoredCircleBackground.getPaint().setColor(getColor(bucket));
            descriptor = new BitmapDrawable(mContext.getResources(), mIconGenerator.makeIcon(getClusterText(bucket)));
            mIcons.put(bucket, descriptor);
        }
//        startMarker.setIcon(getResources().getDrawable(R.mipmap.img_common_patrol_start));
    ...
    private class CreateMarkerTask {
        ...
        private final GeoPoint animateFrom;
         ...
         */
        public CreateMarkerTask(Cluster<T> c, Set<MarkerWithPosition> markersAdded, GeoPoint animateFrom) {
            this.cluster = c;
        ...
        private void perform(MarkerModifier markerModifier) {
            ...
            if (!shouldRenderAsCluster(cluster)) {
                for (T item : cluster.getItems()) {
                    ...
                    if (marker == null) {
                        Marker markerOptions = new Marker(mMap);
                        if (animateFrom != null) {
                            markerOptions.setPosition(animateFrom);
                        } else {
                            markerOptions.setPosition(item.getPosition());
                        }

                        markerOptions.setIcon(item.getBitmapDescriptor());
                        markerOptions.setAnchor(0.5f, 1.0f);
                        //添加额外信息
                        markerOptions.setSnippet(item.getExtraInfo());
                        markerOptions.setInfoWindow(null);
                        //由于特殊情况 这里面添加marker的点击事件
                        markerOptions.setOnMarkerClickListener(mClusterManager);
                        ...
                        marker.setInfoWindow(null);
                        //由于特殊情况 这里面添加marker的点击事件
                        marker.setOnMarkerClickListener(mClusterManager);
                        markerWithPosition = new MarkerWithPosition(marker);
                    }
            ...
            Marker markerOptions = new Marker(mMap);
            markerOptions.setPosition(animateFrom == null ? cluster.getPosition() : animateFrom);
    ...
    private class AnimationTask extends AnimatorListenerAdapter implements ValueAnimator.AnimatorUpdateListener {
        ...
        private AnimationTask(MarkerWithPosition markerWithPosition, GeoPoint from, GeoPoint to) {
